...
def get_args_parser():
    ...
    # * Finetuning params
    # old_results/vlp-batch-32/vlp/best_checkpoint.pth
    ...
    # * Baise params
    parser.add_argument('--output_dir', default='out/GFSLT',
                        help='path where to save, empty for no saving')
    parser.add_argument('--device', default='cuda',
                        help='device to use for training / testing')
    parser.add_argument('--seed', default=42, type=int)
    parser.add_argument('--resume', default='', help='resume from checkpoint')  # data/best_checkpoint.pth
...
def main(args, config):
    ...
    if args.finetune:
        ...
        for k, v in state_dict['model'].items():
            if 'model.conv_2d' in k or 'model.conv_1d' in k:
                k = 'backbone.' + '.'.join(k.split('.')[2:])
                new_state_dict[k] = v
            if 'model.pe' in k or 'model.feedforward' in k or 'model.layer_norm' in k:
                k = 'backbone.' + '.'.join(k.split('.')[2:])
                new_state_dict[k] = v
...
def train_one_epoch(args, model: torch.nn.Module, criterion: nn.CrossEntropyLoss,
    ...
    for step, (src_input, tgt_input) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        out_logits, output, _ = model(src_input, tgt_input)   # prior_encoder_output_dict
...
def evaluate(args, dev_dataloader, model, model_without_ddp, tokenizer, criterion, config, UNK_IDX, SPECIAL_SYMBOLS,
    ...
    with torch.no_grad():
        ...
        for step, (src_input, tgt_input) in enumerate(metric_logger.log_every(dev_dataloader, 10, header)):
            ...
            out_logits, output, prior_encoder_output_dict = model(src_input, tgt_input)  # prior_encoder_output_dict
            total_loss = 0.0
            label = tgt_input['input_ids'].reshape(-1)
    ...
    scbleu = BLEU(force=True)
    bleu_s = scbleu.corpus_score(tgt_pres, [tgt_refs]).score
    metric_logger.meters['belu4'].update(bleu_s)

    new_bleu = bleu(references=tgt_refs, hypotheses=tgt_pres)
    print('All BLEU score：', new_bleu)
